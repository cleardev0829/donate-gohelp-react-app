...
export function getAllPosts() {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get("/api/fundraiser/posts/all");
...
export function getPost(title) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get("/api/fundraiser/post", {
        params: { title },
      });
...
export function getRecentPosts(title) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get("/api/fundraiser/posts/recent", {
        params: { title },
      });
